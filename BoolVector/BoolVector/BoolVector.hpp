...
public:
   ...
   BoolVector XorOperation(const BoolVector& other_) const 
   {
      if (size != other_.size)
         throw SizeMismatchException();  // Исключение при несовпадении размеров

      BoolVector result(size);  // Новый вектор для хранения результата
      result.size = size;
      for (size_t i = 0; i < size; ++i)
         result[i] = (elements[i] != other_.elements[i]);  // XOR операция
