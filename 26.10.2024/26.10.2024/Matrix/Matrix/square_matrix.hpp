...
inline Vector MaxDiagonalElements(SquareMatrix matrix_) 
{
   ...
   */
   ...
   {
      double maxElement = -std::numeric_limits<double>::infinity(); // Íà÷àëüíîå çíà÷åíèå äëÿ ìàêñèìóìà
      int i, j;

      
      if (start < n) {
         i = n - 1; // Íà÷èíàåì ñ ïîñëåäíåé ñòðîêè
         j = start; // Íà÷èíàåì ñ òåêóùåãî ñòîëáöà
      }
      else {
         i = n - param; // Óìåíüøàåì ñòðîêó
         ++param;
         j = n - 1; // Íà÷èíàåì ñ ïîñëåäíåãî ñòîëáöà
      }
