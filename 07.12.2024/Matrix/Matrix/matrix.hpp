...
public:
   ...
   friend fstream& operator>>(fstream& stream, Matrix& matrix_)
   {
      int new_rows = 0;
      int new_columns = 0;
   ...
   friend fstream& operator<<(fstream& stream, const Matrix& matrix_)
   {
       stream << matrix_.rows << " " << matrix_.columns << endl;
